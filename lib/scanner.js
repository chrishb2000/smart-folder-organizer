...
const { checkWrongExtension, checkBrokenFile } = require('./signatures');
...
function scanFolderSync(rootPath, customRules, onProgress, opts) {
  const options = opts || {};
  const checkIntegrity = !!options.checkIntegrity;
  const result = {
    totalFiles: 0,
    totalFolders: 0,
    totalSize: 0,
  ...
  function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (checkIntegrity && stat.size > 0) {
          try {
            if (checkWrongExtension(full)) {
              fileInfo.wrongExtension = true;
              result.wrongExtensions.push(fileInfo);
            }
            const broken = checkBrokenFile(full);
            if (broken.broken) {
              fileInfo.broken = true;
              fileInfo.brokenReason = broken.reason;
              result.brokenFiles.push(fileInfo);
            }
          } catch (e) { /* ignore */ }
        }
...
function hashCacheKey(filePath, size, mtimeMs) {
  return `${filePath}|${size}|${Math.round(mtimeMs)}`;
}

async function hashFileWithCache(filePath, size, mtimeMs, cache) {
  const key = hashCacheKey(filePath, size, mtimeMs);
  if (cache && typeof cache[key] === 'string') {
    return { hash: cache[key], cached: true };
  }
  const hash = await hashFile(filePath);
  if (hash && cache) cache[key] = hash;
  return { hash, cached: false };
}

async function findDuplicates(files, onProgress, cache) {
  const bySize = {};
  for (const f of files) {
  ...
  for (const list of candidates) {
    for (const f of list) {
      ...
      const r = await hashFileWithCache(f.path, f.size, f.modified, cache);
      if (!r.hash) continue;
      if (!f._hash) f._hash = r.hash;
    }
    const byHash = {};
    for (const f of list) {
